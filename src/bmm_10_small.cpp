		...
		if (Jo[ip][i] > Jepsilon && t > tinit) {
			...
			for (int sidx = 0; sidx < dspts[ip].size(); sidx++) {
				if (HAGA == 1) {
					...
					Jo[ip][i] += F[i] * D[i] * alphaLTP * (Cp * exp((dspts[ip][sidx] - t) / tpp) - alphaLTD * fd(Jo[ip][i] / JEE, alpha) * Cd * exp((dspts[ip][sidx] - t) / tpd));
				} else {
		...
		if (t > tinit) {
			for (int sidx = 0; sidx < dspts[j].size(); sidx++) {
				...
				if (HAGA == 1) {
					...
					Jo[i][j] += F[j] * D[j] * alphaLTP * (Cp * exp(-(t - dspts[j][sidx]) / tpp) - alphaLTD * fd(Jo[i][j] / JEE, alpha) * Cd * exp(-(t - dspts[j][sidx]) / tpd));
				} else {
